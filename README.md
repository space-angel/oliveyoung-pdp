...
| 4 충분성 **(구현)** | `U ≥ N_min` **AND** `U/D ≥ R_min` **AND** `S ≥ S_min` — 낮은 쪽이 아니라 높은 쪽. 분모 D 는 **주제를 언급한 작성자**다 | 기본 정책(8 / 0.10 / 8)에서 제품 53개 중 **3개가 침묵**하고, `skinType` 기재 셀 343개 중 **110개(32.1%)**가 셀 단계에서 멈춘다 |
...
pipeline/    v5 — 작업 대상
  ...
  run_v5.py                단계 레지스트리
  test_*.py                계약 테스트 (catalog·policy·ingest·tag·trust·option_norm·gates·sufficiency)
