...
각 `Bxx.md` 는 **자기완결적**이다 (프롬프트 정본 `eval/prompts/concern_candidates_v2.md`, 이 지침과 어긋나면 하네스 파일이 우선) — 역할·규칙·출력 형식·리뷰 전문이 한 파일에 있다. 번들 하나를 작업할 때
...
- `stance` 는 **그 문장이 주제에 대해 긍정인지 부정인지** (`positive` | `negative`). 답 문장과 무관한 절대값이다.
  "구순염 걸렸어요" = `negative`, "다행히 구순염 피해갔네요" = `positive`. 중립·무관한 문장은 근거로 넣지 않는다.
  `support` / `oppose` 는 **옛 규칙**이다 — 쓰면 들여오기에서 배치 전체가 계약 위반으로 표시된다.
- `direction` 은 근거에서 계산한다: 긍정만 → `positive`, 부정만 → `negative`, 둘 다 → `mixed` (반대 1명도 mixed).
