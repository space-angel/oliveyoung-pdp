...
각 `Bxx.md` 는 **자기완결적**이다 (프롬프트 정본 `eval/prompts/concern_candidates_v3.md`, 이 지침과 어긋나면 하네스 파일이 우선) — 역할·규칙·출력 형식·리뷰 전문이 한 파일에 있다. 번들 하나를 작업할 때
...
- `stance` 는 **그 문장이 주제에 대해 긍정 / 부정 / 중립 중 무엇인지** (`positive` | `negative` | `neutral`). 답 문장과 무관한 절대값이다.
  "구순염 걸렸어요" = `negative`, "다행히 구순염 피해갔네요" = `positive`, "지속력은 보통" · "개인차 있을 듯" = `neutral`
  (주제를 말하지만 방향 없음). 주제와 **무관한** 문장은 근거로 넣지 않는다 — 무관과 중립은 다르다.
  `support` / `oppose` 는 **옛 규칙**이다 — 쓰면 들여오기에서 배치 전체가 계약 위반으로 표시된다.
- `direction` 은 근거에서 계산한다: 긍정만 → `positive`, 부정만 → `negative`, 둘 다 → `mixed` (반대 1명도 mixed), 중립만 → `neutral`.
  답(`answer`)은 양쪽 관측을 그대로 서술한다 ("생겼다는 리뷰가 있고, 안 생겼다는 리뷰도 있다").
- **언급하지 않은 리뷰는 어느 쪽 근거도 아니다.** "대부분 문제 없다"처럼 침묵을 부정 증거로 일반화하지 않는다.
