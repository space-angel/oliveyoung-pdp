...
- **주장 골든셋 라벨은 사람이 번들 안에서 블라인드로 만든다** (PER-178). 라벨 1건 = claim(질문–답·조건·방향·근거 인용·실패유형)이고 규격은 `pipeline/golden_contract.py`, 어휘는 `pipeline/failure_taxonomy.json`이다. 도구는 `eval/label_concern_golden.py`(CLI)·`eval/label_concern_golden_web.py`(웹). **Claude가 라벨을 대신 쓰지 않는다** — 골든셋은 모델 산출물을 채점하는 기준이라 모델이 만들면 순환이다. 조건 `null`(무관)과 `"미기재"`(세그먼트)는 다르고, 조건부 주장의 근거는 그 세그먼트 리뷰만이다. v4 골든셋 15문항은 옮기지 않는다 (`eval/reports/v4_golden_migration_per178.json`).
