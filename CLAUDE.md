...
- **리뉴얼은 별개 제품이다** (PER-172). 세대 경계의 키는 `goodsNo`가 아니라 **`(goodsNo, reviewDate)`** 다 — `goodsNo` 교체는 멀티-SKU 제품 36개 중 1개만 리뉴얼이고, 세대가 SKU 코드 안쪽에서 갈리는 제품이 11개다. 세대는 별개 `productId` + 같은 `lineageId`. **`renewalPolicy`에 `null`을 쓰지 않는다** — 정하지 않았으면 `unobserved`로 명시하고, 그 사실이 주장의 `limitation`으로 나간다. 현 스냅샷은 제품 53개 중 `unobserved` 45 · `separate` 6 · `single` 2이고, 계보 L004·L011·L017이 두 세대로 갈려 있다. **리뉴얼 컷의 실효는 0인데 이유가 "정한 게 없어서"가 아니다** — 카탈로그가 `(goodsNo, reviewDate)`로 입수 시점에 이미 세대를 별개 `productId`로 갈라 두기 때문이다. 게이트에 도달할 때는 가를 것이 남아 있지 않다.
...
- **충분성은 세 조건의 AND 다** (PER-186). `U ≥ N_min AND U/D ≥ R_min AND S ≥ S_min` — 낮은 쪽이 아니라 높은 쪽이다. **분모는 주제를 언급한 고유 작성자 `D`이고 셀 크기 `S`가 아니다** — 말하지 않은 사람을 어느 쪽 근거로도 세지 않는다(PER-178). 임계값은 코드 상수가 아니라 `pipeline/policy.py`의 `SufficiencyPolicy`이고 판정 결과의 `meta.sufficiency`에 실린다. `S_min < N_min` · `R_min = 0` · `N_min = 0`은 조건을 조용히 끄는 설정이라 에러다. **반대가 1명이어도 다수 방향으로 뭉개지 않는다** — 컷이 아니라 `single_dissent` 한계로 남긴다. 탈락은 드롭이 아니라 수치가 붙은 `rejected[]` 행이고, 그 목록이 커버리지 실험(PER-199)의 분모다. 근거는 `docs/DECISION_PER186_SUFFICIENCY.md`.
...
- **주장 골든셋 라벨은 사람이 번들 안에서 블라인드로 만든다** (PER-178). 라벨 1건 = claim(질문–답·조건·방향·근거 인용·실패유형)이고 규격은 `pipeline/golden_contract.py`, 어휘는 `pipeline/failure_taxonomy.json`이다. 도구는 `eval/label_concern_golden.py`(CLI)·`eval/label_concern_golden_web.py`(웹). **Claude가 라벨을 확정하지 않는다** — 골든셋은 모델 산출물을 채점하는 기준이라 모델이 확정하면 순환이다. 1차 후보는 다른 모델이 하네스(`eval/concern_candidates.py`)로 만들 수 있되 라벨에 `source`·`candidateId`가 남고, 후보 모델은 생성기(PER-189)·judge(PER-196)와 달라야 하며, 번들마다 사람이 직접 만든 claim(`source=human`)이 1개 이상 있어야 한다. 조건 `null`(무관)과 `"미기재"`(세그먼트)는 다르고, 조건부 주장의 근거는 그 세그먼트 리뷰만이다. **침묵은 근거가 아니다** — 주제를 언급하지 않은 리뷰는 긍정도 부정도 아니고 `silentAuthors`로 따로 센다. 언급 없음을 "대부분 문제 없다"로 일반화하면 `unsupported_claim`이다 (`docs/DECISION_PER178_SILENCE_IS_NOT_EVIDENCE.md`). v4 골든셋 15문항은 옮기지 않는다 (`eval/reports/v4_golden_migration_per178.json`).
