...
| U0 | 주제를 말했지만 방향이 없는 고유 작성자 ("보통이에요") | `neutralAuthors` | 0 |
| D | 주제를 말한 작성자 = U+ ∪ U− ∪ U0 | `spokeAuthors` | 3 |
...
- 방향(`direction`)은 U+와 U−만으로 계산한다. U0(중립)과 S−D(침묵)는 방향에 영향을 주지 않는다. **중립은 침묵이 아니다** —
  주제를 말한 것이므로 D 에 들어간다. 중립을 근거에서 빼면 그 작성자가 침묵으로 잘못 세어진다 (v3, 2026-09-10).
