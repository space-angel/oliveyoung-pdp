...
→ **집계 단위를 `goodsNo`로 두면 안 된다.**

> **정정 (2026-09-04, PER-176).** 이 자리에 원래 "goodsNo로 그룹핑하면 1~7건 상품 139개가
> 생겨 Gate가 대량 실패한다"고 적혀 있었다. **139는 위 블록의 '500건 미달' 수를 잘못 옮긴
> 것이고, 리뷰 1~7건인 `goodsNo`는 26개다.** 방향도 틀렸다 — `goodsNo` 단위로 세면 통과
> 셀이 오히려 많아진다(429 vs `productId` 297). 셀이 잘게 쪼개져 수만 늘기 때문이다.
> 실제 비용은 게이트 실패가 아니라 **파편화**(제품 53개 중 35개가 복수 SKU, 최대 17개)와
> **사장**(8건 미만 SKU 26개에 갇힌 93건)이다. 확정된 계약과 실측은
> [`INPUT_CONTRACT.md`](INPUT_CONTRACT.md) §1, 근거는 `eval/reports/input_contract_per176.json`.

또한 집계 단위는 리뷰 행의 `productKey` 문자열이 아니라 카탈로그가 발급하는 `productId`다
(PER-171) — 이름은 리뉴얼 세대를 못 가르고(계보 3개) v4 매핑에서 실제로 틀린 적이 있다.
